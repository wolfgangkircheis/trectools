...
# Modified to print like trec_eval does. By J. Palotti on Jun 2016.
...
$K = 10;           # Reporting depth for results.
...
if ($topics > 0) {
  ...
  printf "runid\tall\t$runid\n";
  printf "ndcg%d\tall\t%.5f\n",$K,$ndcgAvg;
  printf "err%d\tall\t%.5f\n",$K,$errAvg;

} else {
  #print "$runid,amean,0.00000,0.0000\n";
  printf "runid\tall\t$runid\n";
  printf "ndcg%d\tall\t0.00000\n",$K;
  printf "err%d\tall\t0.00000\n",$K;
}
...
sub topicDone {
  ...
  if ($seen{$topic}) {
    ...
    printf "ndcg%d\t$topicCurrent\t%.5f\n",$K,$ndcg;
    printf "err%d\t$topicCurrent\t%.5f\n",$K,$err;
  }
}
